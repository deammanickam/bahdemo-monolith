...
describe('Blog e2e test', () => {
  ...
  const blogSample = { name: 'take bah', handle: 'yet throughout phooey' };
  ...
  describe('new Blog page', () => {
    ...
    it('should create an instance of Blog', () => {
      cy.get(`[data-cy="name"]`).type('as than onto');
      cy.get(`[data-cy="name"]`).should('have.value', 'as than onto');

      cy.get(`[data-cy="handle"]`).type('connection');
      cy.get(`[data-cy="handle"]`).should('have.value', 'connection');
